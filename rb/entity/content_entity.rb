...
class ContentEntity
  ...
  def make
    opts = @_entopts.dup
    ContentEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TvpHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Content, Hash] the current Content data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TvpHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Content fields)
  def match_get
  ...
  #
  # @param reqmatch [ContentLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Content.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Content, Hash] the loaded Content; raises TvpError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TvpHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
