...
class ContentEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TVP_TEST_CONTENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    content_ref01_data = nil
    if content_ref01_data_raw.length > 0
...
def content_basic_setup(extra)
  ...
  entid_env_raw = ENV["TVP_TEST_CONTENT_ENTID"]
  ...
  if env["TVP_TEST_LIVE"] == "TRUE"
    ...
    client = TvpSDK.new(Helpers.to_map(merged_opts))
  end
