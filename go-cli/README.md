...
boru-driven command-line client **and** interactive REPL for the Tvp
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tvp-cli)
make build
...
./tvp-cli load 1 content            # {id:1} shorthand
./tvp-cli load '{id:1}' content       # explicit match map

# 5. Override the API base URL for a single call
TVP_BASE=https://api.example.com ./tvp-cli load 1 content
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tvp-cli
   ```
...
```sh
./tvp-cli load 1 content          # scalar shorthand for {id:1}
./tvp-cli load '{id:1}' content     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `tvp>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `content`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/tvp-cli`. |
...
(`./tvp-cli <expr>`) and interactively (the REPL), and expressions compose the
