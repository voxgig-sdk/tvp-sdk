// Command Tvp-cli is a CLI + REPL for the Tvp SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TVP_APIKEY (api key), TVP_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TVP_APIKEY carries the API key and
	// TVP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TVP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TVP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTvpSDK(opts)
