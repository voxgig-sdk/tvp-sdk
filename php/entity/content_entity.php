...
class ContentEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ContentEntity($this->_client, $opts);
    }

    /**
     * @param Content|array $args Content data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TvpHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Content|array The current Content data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Content fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TvpHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Content fields).
     */
    ...
    /**
     * Load a single Content.
     *
     * @param ContentLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ContentLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Content|array The loaded Content as an assoc-array at the
     *   SDK boundary; throws TvpError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TvpHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
