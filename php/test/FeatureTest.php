...
class FtCtrl extends TvpControl
{
...
class FtClient
{
    ...
    private TvpUtility $utility;

    public function __construct(TvpUtility $utility, array $options)
    {
    ...
    public function get_utility(): TvpUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TvpContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TvpContext $_ctx, string $_url, array $fetchdef): array {
    ...
    private static function buildUrl(TvpSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TvpError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TvpResponse($fetched);
        }
    ...
    private function fail(TvpContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TvpContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
