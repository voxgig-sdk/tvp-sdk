...
class TvpNetsimFeature extends TvpBaseFeature
{
    ...
    private function _simulate(TvpContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(TvpContext $ctx, array $applied): void
    {
        $client = $this->client;
