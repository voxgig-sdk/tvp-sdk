...
[MCP](https://modelcontextprotocol.io) server exposing the Tvp SDK as
two agent tools — `tvp_list` and `tvp_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tvp-mcp)
make build

# 2. Provide credentials via the environment
export TVP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user tvp \
  -- /absolute/path/to/tvp-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./tvp-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// tvp_list: first page of records
{ "entity": "content" }
{ "entity": "content", "query": { } }

// tvp_load: one record by id
{ "entity": "content", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tvp-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user tvp \
     -- "$PWD"/dist/*/tvp-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `tvp_list` and `tvp_load` tools now appear
   in new sessions. Ask the agent to *"list content using tvp"*
   and it calls `tvp_list` with `{"entity":"content"}`.
...
```sh
export TVP_APIKEY=sk_live_xxx            # API key
export TVP_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `tvp_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `tvp_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TVP_APIKEY` | API key sent with every request. |
| `TVP_BASE` | Optional override of the API base URL. |
...
```sh
./tvp-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"tvp_load","arguments":{"entity":"content","query":{"id":1}}}}'
```
