...
function isRunnable(code) {
    return (/new\s+TvpSDK\b/.test(code) ||
        /\bTvpSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+TvpSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/TvpSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
