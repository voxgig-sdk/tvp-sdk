...
class TvpTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TvpTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TvpTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TvpTestRunner.getenv("TVP_TEST_LIVE")
        override = TvpTestRunner.getenv("TVP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TvpTestRunner.getenv(key)
        ...
        explain = TvpTestRunner.getenv("TVP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TVP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TvpTestRunner._test_control is not None:
            return TvpTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TvpTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TvpTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TvpTestRunner.load_test_control()
...
def load_env_local():
    TvpTestRunner.load_env_local()


def env_override(m):
    return TvpTestRunner.env_override(m)


def entity_data(v):
    return TvpTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TvpTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TvpTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TvpTestRunner.load_test_control()


def live_client_options():
    return TvpTestRunner.live_client_options()


def live_delay_ms():
    return TvpTestRunner.live_delay_ms()
