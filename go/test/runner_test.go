...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TVP_TEST_LIVE") == "TRUE" ||
		os.Getenv("TVP_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TVP_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.TvpSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.TvpError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.TvpSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.TvpError{Msg: msg, Code: code}
}
