...
import (
	...
	omni "github.com/voxgig-sdk/tvp-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TvpError); is {
				out := map[string]any{
					"name":    "TvpError",
...
func omniCtx(arg any, client *sdk.TvpSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TvpSDK); is {
			client = live
