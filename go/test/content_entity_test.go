...
func contentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"content01", "content02", "content03", "token01", "token02", "token03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("TVP_TEST_CONTENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TVP_TEST_CONTENT_ENTID"])
	...
	if env["TVP_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewTvpSDK(core.ToMapAny(mergedOpts))
	}
