...
module TvpUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = TvpResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TvpResponse.new(fetched)
    else
