...
//    only - a typed TvpContext there would make every ctx
...
final class TvpOmni
{
    ...
    public static function err_from_map($m): ?TvpError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TvpError(is_string($code) ? $code : '', $msg);
    }
