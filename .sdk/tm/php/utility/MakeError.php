...
class TvpMakeError
{
    public static function call(?TvpContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TvpContext([], null);
        }
        $op = $ctx->op ?? new TvpOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TvpResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TvpError) ? $err->msg : (string)$err;
        $msg = "TvpSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new TvpError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof TvpError) {
