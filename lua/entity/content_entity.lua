...
local ContentEntity = {}
ContentEntity.__index = ContentEntity


function ContentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ContentEntity)
  self._name = "content"
  self._client = client
...
function ContentEntity:get_name()
  return self._name
end


function ContentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ContentEntity.new(self._client, opts)
end
...
function ContentEntity:mark_deleted()
  self._deleted = true
end


function ContentEntity:deleted()
  return true == self._deleted
end


function ContentEntity:data_set(args)
  if args ~= nil then
...
function ContentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ContentEntity:match_set(args)
  if args ~= nil then
...
function ContentEntity:match_get()
...
function ContentEntity:stream(action, args, callopts)
...
---@param reqmatch ContentLoadMatch
---@param ctrl? table
---@return Content
---@return string? err
function ContentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ContentEntity:_run_op(ctx, post_done)
  local utility = self._utility
